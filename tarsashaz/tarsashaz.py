...
class tul_hanyad(osv.osv):
    ...
    def _felszolit_szamol(self,cr,uid,ids,field_name,arg,context=None):
        ...
        szmlal=felsz.search_count(cr,uid, [('tulaj', '=', sajat_id),('felsz_status','<>','rend')], context=context)
        res={}
        res[sajat_id]=szmlal
        return res
    ...
    _columns = {'tulhanyad':fields.integer(string='Tul. hanyad:'),
                ...
                'alapito_ids': fields.many2many('ir.attachment', 'class_ir_attachments_alap', 'alapito_id', 'attachment_id', 'Alapító okiratok:', help=''),
                'biztositas_ids': fields.many2many('ir.attachment', 'class_ir_attachments_biztos', 'biztosito_id', 'attachment_id', 'Biztosítási kötvények:', help=''),
                ...
                'felszolitas':fields.function(_felszolit_szamol,string='Felszólítások',type='integer', store=False, help=''),
                'muszakis':fields.many2one('res.partner', 'Műszaki előadó', domain="[('parent_id.name','ilike','újlipótv')]"),
                'konyvelo': fields.many2one('res.partner', 'Könyvelő',domain="[('parent_id.name','ilike','újlipótv')]"),
                }
