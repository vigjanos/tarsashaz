...
class tarh_havijel_haz(osv.osv):
    ...
    _columns = {
        ...
        'th_szamlatul': fields.many2one('res.partner', 'Társashaz', required=True),
        'sor_id': fields.one2many('tarh.havijel.haz.sor', 'havijel_id', 'kapocs2'),
        'bankszamla': fields.many2one('res.partner.bank', 'Bankszamla', domain="[('partner_id', '=',th_szamlatul)]",
                                      help="Listazni kivant bankszamla, vagy penztar,ha ures akkor mindet listazza"),
    }
    ...
    def kiszamol (self, cr, uid, ids, context=None):
        ...
        _bankszamla = self.browse(cr, uid, ids, context=None).bankszamla.id
        ...
        '''
        Megkeressük az összes bankszámláját a társasháznak, ha megadtunk egy bankszámlát, akkor csak azt, ha nem adtunk
        meg egyet sem akkor az összes számláját, pénztárát a háznak
        '''
        if _bankszamla:
            bankszamlak = [_bankszamla]
        else:
            bankszamlak = self.pool.get('res.partner.bank').search(cr, uid, [('partner_id', '=', _szamlatul)],
                                                                   context=None)
        for bankszamla in bankszamlak:
            ...
            conn_str = "select sum(terheles) as terheles, sum(my_report.jovairas) as jovairas, tarh_tranzakcio.name as tarh_tranzakcio" \
                       " from my_report join tarh_tranzakcio on tarh_tranzakcio=tarh_tranzakcio.id where th_szamlatul=" + str(
                _szamlatul) + "" \
                              " and erteknap between '" + _kezdatum + "' and '" + _vegdatum + "' and bankszamla_thaz=" + str(
                bankszamla) + " group by tarh_tranzakcio.name order by tarh_tranzakcio.name"
            cr.execute(conn_str)
...
class tarh_haz_lakoegy(osv.osv):
    ...
    def lakoegyenlegek (self, cr, uid, ids, contex=None):
        ...
        aktiv_lako = ref_res_partner.search(cr, uid, [('parent_id', '=', _tarsashaz), ('active', '=', True)],
                                            context=None)
        nem_aktiv_lako = ref_res_partner.search(cr, uid, [('parent_id', '=', _tarsashaz), ('active', '=', False),
                                                          ('alb_eladas', '>=', _vegdatum)], context=None)
